...
**Where the horses actually are (default assumption for prose): Firsthold.** The rule below says what the Ascend *permits*; this says what is *normally done*, and the two are not the same. **There is no practical use for a horse at Blackspire.** It is a keep on a summit — nothing inside it is far enough away to ride to, there is no ground to work and nowhere to go, and a stable up there means hauling fodder up a fortified mountain path in perpetuity. So riders, couriers and travelling parties **stable in Firsthold and climb**, and any character arriving at the keep should be assumed to have done that unless there is a reason otherwise.

**The exception is display, not transport.** A great house making a ceremonial arrival or departure leads its horses up so the party passes the upper gate mounted and under banners — Ch.57's Vaelmar departure is exactly this. It is staging. It is slow and expensive and nobody does it for an ordinary errand.
